...
Deploid.prototype._deployNode = function( node, done ) {
	...
	var packageFile = self._tmpDirRemote + self._packageCacheFile.replace( /^.*\//, '' );
	...
	async.series( [
		...
		function( done ) {
			self._runSshCmd( node, 'sudo tar -xzf --directory ' + self._tmpDirRemote + ' ' + packageFile, done );
		},
		function( done ) {
...
Deploid.prototype._prepareTmpDir = function( done ) {
	...
	this._tmpDirRemote = '/tmp/daemonixDeploy/' + this._tmpDir.replace( /^.*\//, '' ) + '/';
