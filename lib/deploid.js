...
Deploid.prototype._createUpstartConfFile = function( upstartFile, done ) {
	...
	async.series( [
		function( done ) {
			getNodeVersion( self._tmpDirVersion + '/src/package.json', function( err, nodeVersion ) {
				if ( err ) {
					done( err );
					return;
				}
...
Deploid.prototype._deployNode = function( node, done ) {
	...
	var tmpUpstartConf = this._tmpDir.path + '/' + self._config.daemon + '-' + process.pid + '.conf';
...
Deploid.prototype._prepareTmpDir = function( done ) {
	...
	this._tmpDir = new tmp.Dir().path;
	this._tmpDirVersion = this._tmpDir + '/' + this._config.version;
