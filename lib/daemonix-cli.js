...
var DaemonixCli = function() {
	...
	self._program
		...
		.action( function( appName, envName, version, program ) {
			...
			self.deploy( appName, envName, version, program.yes || false, program.forceRebuild || false, function( err ) {
				if ( err ) {
					console.error( err );
					process.exit( 1 );
				}
...
DaemonixCli.prototype.deploy = function( appName, envName, version, yes, forceRebuild, done ) {
