...
var DaemonixCli = function() {
	...
	self._program
		...
		.description( 'Add target deployment nodes (comma separated list of FQDNs or IPs) for the specified environment for an application already in the repository.' )
		.action( function( appName, envName, version ) {

			self.deploy( appName, envName, version, function( err ) {
				if ( err ) {
					console.error( err );
					process.exit( 1 );
				}
...
DaemonixCli.prototype.deploy = function( appName, envName, version, done ) {
	...
	self._readApp( appName, function( app ) {
		...
		console.log( "Deploymentn configuration:\n\n" + JSON.stringify( config, null, 4 ) + "\n" );
