...
(function() {
	...
	var Daemonix = module.exports = function( deps ) {
		...
		self._workers = {
			count:           ( workers.count === 'auto' ||
							   workers.count > 0 ?
							   workers.count :
							   defaultConfig.workers.count ),
			restartTimeout:  ( workers.restartTimeout > 1000 ?
							   workers.restartTimeout : 1000 ),
			shutdownTimeout: ( workers.shutdownTimeout > 30000 ?
							   workers.shutdownTimeout : 10000 )
		};
