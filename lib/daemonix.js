...
class Daemonix {
  ...
  _startMaster() {
    ...
    let targetWorkerCount;
    switch (this._workersConfig.count) {
      case 'auto':
        targetWorkerCount = Math.max(2, this._os.cpus().length);
        break;
    ...
    let workerCount = () => {
      ...
      for (let i in this._cluster.workers) {
        if (Object.prototype.hasOwnProperty.call(this._cluster.workers, i)) {
          count++;
        }
      }
    ...
    this.terminationHandler = () => {
      let force = () => {
        ...
        for (let pid in this._cluster.workers) {
          if (
            Object.prototype.hasOwnProperty.call(this._cluster.workers, pid)
          ) {
      ...
      for (let pid in this._cluster.workers) {
        if (Object.prototype.hasOwnProperty.call(this._cluster.workers, pid)) {
